...
func HttpResult(r *http.Request, w http.ResponseWriter, resp interface{}, err error) {
	...
	if err == nil {
		...
		//错误返回
		errcode := ercode.SERVER_COMMON_ERROR
		errmsg := "服务器开小差啦，稍后再来试一试"

		causeErr := errors.Cause(err)                   // err类型
		if e, ok := causeErr.(*ercode.ErrorEntry); ok { //自定义错误类型
			//自定义CodeError
			...
			// 	if ercode.IsCodeErr(grpcCode) { //区分自定义错误跟系统底层、db等错误，底层、db错误不能返回给前端
			// 		errcode = grpcCode
...
func ParamErrorResult(r *http.Request, w http.ResponseWriter, err error) {
	httpx.WriteJson(w, http.StatusBadRequest, Error(ercode.REUQEST_PARAM_ERROR, err.Error()))
}
